<?php
error_log('DEBUG: coordinatorWeeklyReportAjax.php loaded');
...
if (isset($_SESSION['coordinator_user'])) {
    $_SESSION['user_id'] = $_SESSION['coordinator_user'];
    $_SESSION['user_type'] = 'coordinator';
}


// Check if user is logged in
error_log('DEBUG: SESSION user_type: ' . ($_SESSION['user_type'] ?? 'unset'));
error_log('DEBUG: SESSION user_id: ' . ($_SESSION['user_id'] ?? 'unset'));
...
function getWeeklyReports() {
    error_log('DEBUG: getWeeklyReports POST: ' . print_r($_POST, true));
    error_log('DEBUG: getWeeklyReports called. POST: ' . print_r($_POST, true));
    error_log('DEBUG: getWeeklyReports POST: ' . print_r($_POST, true));
    global $conn;
    ...
    if ($studentId && $studentId !== 'all') {
        ...
        // Show all students under this coordinator
        $coordinatorId = $_SESSION['user_id'] ?? null;
        if ($coordinatorId) {
            $query .= " AND r.interns_id IN (SELECT itd.INTERNS_ID FROM intern_details itd JOIN internship_needs ins ON itd.HTE_ID = ins.HTE_ID WHERE ins.COORDINATOR_ID = ?)";
    ...
    if ($weekStart && $weekEnd && $weekStart === $weekEnd) {
        // If a single date is selected, find reports where the date is within the week range
        $query .= " AND (? BETWEEN r.week_start AND r.week_end)";
        $params[] = $weekStart;
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        if ($currentReportId !== $row['report_id']) {
            ...
            // Process per-day content and ensure all days are present
            $daysOfWeek = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday'];
            $contentPerDay = array_fill_keys($daysOfWeek, '');
            if (!empty($row['report_content'])) {
                $contentData = json_decode($row['report_content'], true);
                if (is_array($contentData)) {
                    foreach ($daysOfWeek as $day) {
                        if (isset($contentData[$day])) {
                            $contentPerDay[$day] = $contentData[$day];
                        }
                    }
                } else {
                    // Fallback for old format
                    $contentPerDay['monday'] = $row['report_content'];
                }
            }
            $currentReport['contentPerDay'] = $contentPerDay;

            // Always include each day's description field
            foreach ($daysOfWeek as $day) {
                $descField = $day . '_description';
                $currentReport[$day . 'Description'] = isset($row[$descField]) ? $row[$descField] : '';
            }
